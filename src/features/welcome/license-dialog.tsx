...
export function LicenseDialog({ isOpen, onClose }: LicenseDialogProps) {
  return (
    <FlowDialog open={isOpen} onOpenChange={(open) => !open && onClose()} className="p-5 select-none max-h-[85vh] flex flex-col">
      <FlowPopupHeader
        title="Apache License 2.0"
        description="Flow is open-source software licensed under the Apache License, Version 2.0."
        onClose={onClose}
        className="flex-shrink-0"
      />

      <div className="flex-1 overflow-hidden my-4 border rounded-2xl bg-muted/30">
        <ScrollArea className="h-64 sm:h-80 w-full p-4">
          <pre className="text-[10px] leading-relaxed text-muted-foreground font-mono whitespace-pre-wrap">
{`                                 Apache License
...
`}
      ...
      <FlowPopupFooter className="flex-shrink-0">
